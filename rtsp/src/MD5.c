...
/* message digest */
/* context */
void MD5_Final (MD5 digest, MD5_CTX *context)
{
    UCHAR bits[8];
    UINT index, padLen;
...
void MD5Buffer(void *_buffer, size_t size, MD5 digest)
{
    MD5_CTX context;
    MD5_Init(&context);
    ...
    while (size > uint_max) {
        MD5_Update(&context, (UCHAR*)buffer, UINT32_MAX);
        size -= uint_max;
        buffer = (UCHAR*)buffer + uint_max;
    }

    MD5_Update(&context, (unsigned char*)buffer, (UINT)size);
    MD5_Final(digest, &context);
}
...
int MD5Test(void)
{
    ...
    MD5 digest;
    MD5_Init(&context);
    MD5_Update(&context, (unsigned char*)test_str, (UINT)strlen(test_str));
    MD5_Final(digest, &context);
